export function TriviaAnswersEditor({ isBeingEdited, answer, handleTextChange, handleIsCorrectChange, handleDelete, handleAdd }) {
    return (
        <div className="m-3">
            ...
            {(handleDelete && isBeingEdited) && <button onClick={() => handleDelete(answer._id)} className='bg-red-500 text-white rounded-md py-1 px-3 mx-2'>Borrar respuesta</button>}
            {(handleAdd && isBeingEdited) && <button onClick={() => handleAdd(answer)} className='bg-green-500 text-white rounded-md py-1 px-3 mx-2'>Agregar respuesta</button>}
        </div>
    );
}